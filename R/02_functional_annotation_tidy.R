...
# Function to extract IMG scaffold IDs embedded in gene IDs
...
img_cog <- read_tsv(file = paste0(taxon_oid, ".a.cog.txt"),
                    col_names = c("gene_id", "cog_id", "percent_identity",
                                  ...
                                  "bit_score"),
                    quote = "") %>%
  mutate(scaffold_id = extract_scaffold_id(gene_id))
...
# Join COG, KO tables with scaffold coverage information
